...
if [ "$_old_kerberos" != "$_kerberos" ]; then
    ...
    for KERBSERVICE in $_old_kerberos; do
        ...
        if ! [[ " $_kerberos " == *" $KERBSERVICE "* ]]; then
            ...
            vault policy delete kerberos${KERBSERVICE}
            rm -f policies/kerberos${KERBSERVICE}.hcl
        fi
    done
else
    for KERBSERVICE in $_old_kerberos; do
...
for KERBSERVICE in $_kerberos; do
    ...
    COMPATPOLICIES="$COMPATPOLICIES kerberos$KERBSERVICE"
    CHANGED=false
    ...
    POLICYNAME=kerberos${KERBSERVICE}
    ...
    for ROLE in $roles; do
        ...
        if ! $ENABLED || $CHANGED || [ "$scopes" != "$old_scopes" ] || [ ! -f policies/${POLICYNAME}.hcl ]; then
            ...
            vault write $VPATH/role/$ROLE - \
                ...
                groups_claim="" \
                oidc_scopes="$scopes" \
                policies=default,${POLICYNAME},tokencreate \
...
EOF
        ...
        for POLICY in oidc; do
            POLICYISSUER="${POLICY}-${ISSUER}"
            POLICYNAME="${POLICY}${ISSUER}_${ROLE}"
            ...
            sed -e "s,<vpath>,secret/oauth-$ISSUER," -e "s/<${TEMPLATEPOLICY}>/$ACCESSOR/" -e "s/@<domain>/$policydomain/" -e "s/<role>/$ROLE/" $LIBEXEC/${TEMPLATEPOLICY}policy.template >>policies/${POLICYNAME}.hcl.new
        done
    done
...
EOF
	...
	ACCESSOR="`vault read sys/auth -format=json|jq -r '.data."'$POLICYISSUER'/".accessor'`"
	sed -e "s,<vpath>,$VPATH," -e "s/<${TEMPLATEPOLICY}>/$ACCESSOR/" -e "s/@<domain>/$policydomain/" $LIBEXEC/${TEMPLATEPOLICY}policy.template >>policies/${POLICY}.hcl.new
    done
done
